...
function sliderFeatures()
{
    ...
    while ($row = fetch_array($query)) {
        $sliderFeatures = <<<DELIMETER
                <div class="col-md-2 product pt-md-5 col-lg-4">
                    ...
                    <div class="cart-details">
                        ...
                        <div class="pro-price py-2">
                            <h5>
                                <small><s class="text-secondary">P{$row['productPrice']}</s></small>
                                <span>P{$row['productPriceDiscount']}</span>
                            </h5>
                        </div>
                        <div class="cart mt-4">
...
{
    $query = query("SELECT * FROM products WHERE productCategoryID = '12'");
    confirm($query);
                ...
                <div class="row py-3">
                    ...
                    <div class="col-md-9 p-0 py-4 py-md-0">
                        <div class="px-4">
                            ...
                            <h5>
                                <small><s class="text-secondary">P{$row['productPrice']}</s></small>
                                <span class="text-color">P{$row['productPriceDiscount']}</span>
                            </h5>
                        </div>
                    </div>
                </div>
...
function addProducts()
{
    if (isset($_POST['add'])) {
        ...
        $productImageTemporary = escape_string($_FILES['image']['tmp_name']);
        $productImagePath = "images/" . $productImage;
        $rand = rand(1, 100);
...
function updateProducts()
{
    if (isset($_POST['update'])) {
        ...
        $query .= "productPriceDiscount = '{$productPriceDiscount}',";
        $query .= "productImage = '{$productImage}',";
        $query .= "productDescription = '{$description}',";
        $query .= "productShortDescription = '{$short_desc}',";
